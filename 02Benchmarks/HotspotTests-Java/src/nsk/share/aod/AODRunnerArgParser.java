...
public class AODRunnerArgParser extends ArgumentParser {
    ...
    public String getTargetApp() {
        if (!vmoptions.containsKey(targetAppParam))
            throw new TestBug("Target application isn't specified");

        return vmoptions.getProperty(targetAppParam);
    }

    public String getTestedJDK() {
        if (!vmoptions.containsKey(testedJdkParam))
            throw new TestBug("Tested JDK isn't specified");

        return vmoptions.getProperty(testedJdkParam);
    }

    public String getJavaOpts() {
        return vmoptions.getProperty(javaOptsParam, "");
    }
