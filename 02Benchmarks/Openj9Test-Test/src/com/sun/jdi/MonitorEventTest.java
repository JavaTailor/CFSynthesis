...
class myThread extends Thread {
    public void run() {
        synchronized(MonitorTestTarg1.startingMonitor) {
            MonitorTestTarg1.startingMonitor.notify();
        }

        // contended enter wait until main thread release monitor
        MonitorTestTarg1.aboutEnterLock = true;
        synchronized (MonitorTestTarg1.endingMonitor) {
        }
    }
}
